...
import { esc, fmtDate, fmtPhone, fmtLang, fmtNeuro } from '../format'
  ...
  const load = async () => {
    try {
      ...
      renderTable<Attempt>(tbl, [
        ...
        { key: 'dominant', label: 'Доминанта', sortable: true, render: r => r.dominant ? `<b>${esc(fmtNeuro(r.dominant))}</b>` : '<span class="muted">—</span>' },
